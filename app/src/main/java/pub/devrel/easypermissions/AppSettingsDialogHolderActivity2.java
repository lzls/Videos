...
public class AppSettingsDialogHolderActivity2 extends AppCompatActivity
    ...
    public static Intent createShowDialogIntent(Context context, AppSettingsDialog dialog) {
        Intent intent = new Intent(context, AppSettingsDialogHolderActivity2.class);
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == Dialog.BUTTON_POSITIVE) {
            ...
            setResult(Activity.RESULT_CANCELED,
                    new Intent().putExtra(Consts.KEY_FROM_APP_DETAILS_SETTING_SCREEN, false));
            finish();
        } else {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        setResult(resultCode, data.putExtra(Consts.KEY_FROM_APP_DETAILS_SETTING_SCREEN, true));
        finish();
    }
}